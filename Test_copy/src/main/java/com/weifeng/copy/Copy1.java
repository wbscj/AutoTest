...
public class Copy1 {
    private static String CONF = "CONF.txt";
    ...
    public static void verify() throws IOException {
        ...
        //获得压力机数量
        splitNumber = (int)Files.lines(Paths.get(CONF)).count();
        while (true){
            if (splitNumber <= 0){
                System.out.print("压力机数量为 0 时无法拆分文件！");
    ...
    public static void split(String filePath,int splitNumber) throws IOException {
        ...
        LineNumberReader readerConf = new LineNumberReader(new FileReader(CONF));
        for (int i = 1; i <= splitNumber; i++){
            ...
            //读取压力机的每一行
            String line = readerConf.readLine();
            String path = new File(line).getParent();
            File file = new File(path);
            if (!file.exists()){
                System.out.println();
                System.out.printf("开始创建路径：%s \n",path);
                file.mkdirs();
            }
            FileWriter long_write = new FileWriter(line);
            System.out.println("开始写入 "+line);
            while (len <= count){
